...
df = pd.read_csv("C:\\Users\\Avinash\\Desktop\\DataScience_AI\\Solutions\\Ensemble\\Diabeted_Ensemble.csv")
...
df = pd.get_dummies(df, columns = ["Class_variable"], drop_first = True)

df.head()

# Input and Output Split
predictors = df.loc[:, df.columns!="Class_variable_YES"]
type(predictors)

target = df["Class_variable_YES"]
type(target)
...
train_x, train_y = df[:600], df.Class_variable_YES[:600]
test_x, test_y = df[600:], df.Class_variable_YES[600:]
