...
df = pd.read_excel("C:\\Users\\Avinash\\Desktop\\DataScience_AI\\Solutions\\Ensemble\\Coca_Rating_Ensemble.xlsx")
...
#Bean_Type has varying data types 
df.Bean_Type = LabelEncoder().fit_transform(df.Bean_Type.astype(str))
df.Bean_Type = LabelEncoder().fit_transform(df.Bean_Type)

#Origin has varying data types
df.Origin = LabelEncoder().fit_transform(df.Origin.astype(str))
df.Origin = LabelEncoder().fit_transform(df.Origin)
#df.columns = df.columns.str.replace(' ', '_')

#converting continuous data to categorical data
#df['Rating'] = pd.cut(df.Rating, bins=5, labels=np.arange(5), right=False)
df['Rating'] = pd.cut(df.Rating, bins=[0.5,1.5,2.5,3.5,4.5,5.5], labels=[1,2,3,4,5], right=False)
df.head()
...
train_x, train_y = df[:1430], df.Rating[:1430]
test_x, test_y = df[1430:], df.Rating[1430:]
