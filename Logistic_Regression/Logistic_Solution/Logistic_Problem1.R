...
#affairs$naffairs <- cut(affairs$naffairs, breaks = c(0,0,Inf), labels = c(0,1))
#affairs$naffairs<-ifelse(affairs$naffairs==0,0,1)

#converting other than zero values to 1 in the
...
#Null deviance means error for baseline model i.e. model build on mean output value. 
