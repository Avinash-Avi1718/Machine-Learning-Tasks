...
colnames(data)
data <- data1
data <- data[ , c(-5,-6,-8,-9)] # Removing the columns

# Preparing a linear regression 
mod_lm <- lm(Clicked_on_Ad ~ ., data = data)
summary(mod_lm)
...
model <- glm(Clicked_on_Ad ~ ., data = data, family = "binomial")
summary(model)
...
confusion <- table(prob > 0.5, data$Clicked_on_Ad)
confusion
...
library(caret)
# Confusion Matrix
confusionMatrix(factor(data$Clicked_on_Ad, levels = c(0, 1)), factor(pred_values, levels = c(0, 1)))
...
# The output of sigmoid function lies in between 0-1
fullmodel <- glm(Clicked_on_Ad ~ ., data = data1, family = "binomial")
summary(fullmodel)
...
optCutOff <- optimalCutoff(data1$Clicked_on_Ad, prob_full)
optCutOff
...
misClassError(data1$Clicked_on_Ad, prob_full, threshold = optCutOff)


# ROC curve
# Greater the area under the ROC curve, better the predictive ability of the model
plotROC(data1$Clicked_on_Ad, prob_full)


# Confusion Matrix
predvalues <- ifelse(prob_full > optCutOff, 1, 0)

results <- confusionMatrix(predvalues, data1$Clicked_on_Ad)

sensitivity(predvalues, data1$Clicked_on_Ad)
confusionMatrix(actuals = data1$Clicked_on_Ad, predictedScores = predvalues)
