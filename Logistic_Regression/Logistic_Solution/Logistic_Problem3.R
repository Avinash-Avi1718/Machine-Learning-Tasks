...
library(caret)
# Confusion Matrix
confusionMatrix(factor(data$Result, levels = c(0, 1)), factor(pred_values, levels = c(0, 1)))
...
# The output of sigmoid function lies in between 0-1
fullmodel <- glm(Result ~ ., data = data1, family = "binomial")
summary(fullmodel)
...
optCutOff <- optimalCutoff(data1$Result, prob_full)
optCutOff
...
misClassError(data1$Result, prob_full, threshold = optCutOff)


# ROC curve
# Greater the area under the ROC curve, better the predictive ability of the model
plotROC(data1$Result, prob_full)


# Confusion Matrix
predvalues <- ifelse(prob_full > optCutOff, 1, 0)

results <- confusionMatrix(predvalues, data1$Result)

sensitivity(predvalues, data1$Result)
confusionMatrix(actuals = data1$Result, predictedScores = predvalues)
