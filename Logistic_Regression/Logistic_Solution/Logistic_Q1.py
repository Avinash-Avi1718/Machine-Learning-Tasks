...
#Importing Data
Affairs = pd.read_csv("C:\\Users\Avinash\OneDrive\Desktop\DataScience_AI\Solutions\Logistic_Regression\\Affairs.csv", sep = ",")

#removing CASENUM
Affairs.info()
c1 = Affairs.drop('Unnamed: 0', axis = 1)
c1.head(11)
naffairs_binary = []
for i in range(len(c1.naffairs)):
    if c1.naffairs[i]>0:
        naffairs_binary.append(1)
    else:
        naffairs_binary.append(0)
c1['naffairs'] = naffairs_binary
c1.naffairs[2]
c1.describe()
c1.isna().sum() #No missing values
c1.info()
#############################################################

# Model building 
# import statsmodels.formula.api as sm
logit_model = sm.logit('naffairs~kids+vryunhap+unhap+avgmarr +hapavg+vryhap+antirel +notrel+slghtrel+smerel+vryrel+yrsmarr1+yrsmarr2+yrsmarr3+yrsmarr4+yrsmarr5+yrsmarr6', data = c1).fit()
