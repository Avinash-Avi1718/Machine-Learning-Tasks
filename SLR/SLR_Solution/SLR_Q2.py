...
df = pd.read_csv("C:\\Users\\Avinash\\OneDrive\\Desktop\\DataScience_AI\\Solutions\\SLR\\delivery_time.csv")
...
plt.bar(height = df.Delivery_Time, x = np.arange(1, 22, 1))
plt.hist(df.Delivery_Time) #histogram
plt.boxplot(df.Delivery_Time) #boxplot

plt.bar(height = df.Sorting_Time, x = np.arange(1, 22, 1))
...
plt.legend(['Predicted line', 'Observed Delivery_Time'])
plt.show()
...
######### Model building on Transformed Delivery_Time
# Log Transformation
...
np.corrcoef(df.Sorting_Time, np.log(df.Delivery_Time)) #correlDelivery_Timeion
