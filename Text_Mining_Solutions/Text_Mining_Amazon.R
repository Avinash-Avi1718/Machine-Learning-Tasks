...
######### Amazon URL ###########
aurl <- "https://www.amazon.in/New-Apple-iPhone-11-128GB/product-reviews/B08L8CXFZ1/ref=cm_cr_arp_d_paging_btm_next_2?ie=UTF8&reviewerType=all_reviews&pageNumber"

amazon_reviews <- NULL

#Retrieving class in the htms tag where reviews are lying
for (i in 1:20){
...
write.table(amazon_reviews,"iphone_11.txt")
getwd()
...
# Removing unwanted terms ot not useful terms
x1 <- tm_map(x1, removeWords, c('phone','iphone','will','amazon','apple'))
x1 <- tm_map(x1, stripWhitespace)
...
#### Bigram ####
#Installed java for RWeks to work
...
#Go to last line of the file, enter and save file again and then load
